...
def transcribe(file: UploadFile = File(...)):
    ...
    try:
        ...
        for ev in note_events:
            notes.append({
                "midi": int(getattr(ev, "pitch_midi", getattr(ev, "pitch", 60))),
                "start": round(float(getattr(ev, "start_time_s", getattr(ev, "start", 0.0))), 3),
                "end": round(float(getattr(ev, "end_time_s", getattr(ev, "end", 0.0))), 3),
                "dur": round(max(0.0, float(getattr(ev, "end_time_s", getattr(ev, "end", 0.0))) - float(getattr(ev, "start_time_s", getattr(ev, "start", 0.0)))), 3),
            })
