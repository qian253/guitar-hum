...
- **降级**：Karplus-Strong 拨弦物理建模（零采样、不依赖网络），**经典结构：周期延迟 + 两点平均滤波 0.5(x+x·z⁻¹) + 阻尼 0.99**（v2.11.2/2.11.4，任意频率增益 ≤1 不可能自激），环外 +3dB@7kHz 提亮（不影响稳定性），默认音量 0.65。诊断面板「采样」行可看到当前是真实采样还是降级拨弦、「采样率」行显示播放上下文采样率、「最近回放」行显示触发数。
