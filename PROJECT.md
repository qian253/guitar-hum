...
- **首选**：Tone.js `Sampler`，采样托管在 `gleitz.github.io/midi-js-soundfonts/FluidR3_GM/`（**国内常被墙**）。
- **降级**：Karplus-Strong 拨弦物理建模（零采样、不依赖网络，音色接近尼龙弦），`toneOsc()` 函数。
- **关键修复**：所有播放函数 `await ensureAudioStarted()`（同时唤醒 Tone + 原生 AudioContext，**必须在用户手势内**）；`waitSampler()` 最多等 2 秒（之前 6 秒导致国内用户感觉「没声音」）。
...
## 七、下一步 TODO / 优化项（已确认但未做）

- 【高价值】用 basic-pitch 的 **振幅做「节拍强度」**权重（重心音 = 时长×振幅），当前重心只用了时长。
- 【高价值】展示**前两名候选调**（如「大概率 B 小调 62% / 也可能是 D 大调」），别硬给一个答案。
- 【中】Essentia 从「仅展示一致/不一致」升级为「参与投票」。
- 【中】重心/音级分布权重 0.04 形同虚设，配合振幅节拍强度后可调回有意义的值。
