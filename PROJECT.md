...
- **v2.12.0 听旋律双路径（节奏/重音一比一 + 音色可切换）**：采样器就绪时**逐音真实采样触发**——音色随「音色」栏切换（尼龙/钢弦/钢琴），每音起音/时值取真实转录、时长不压过下一音，力度用共享的 `computeNoteVels`（basic-pitch 振幅动态范围 0.55+0.45×归一振幅，快速模式从录音 PCM 窗口 RMS）。采样器未就绪时落 **K-S 预渲染缓冲**保底（v2.11.8 路径，零触发依赖）。此前采样器路径「只剩最后一个音」的谜底是音符逆序（v2.11.15），排序修复后采样器路径回归安全。
