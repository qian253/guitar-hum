...
- **v2.10.0 音量加固**：K-S 降级默认音量 0.25/0.3 → 0.4，采样器 -6dB → -3dB，调度时间早于 currentTime 时立即发声（防止 `when` 已过导致静默）。
- **测试声音按钮**：录音屏，点击立即用 Karplus-Strong 发声（不等采样），并显示 Tone.Master 音量/静音诊断。诊断面板新增「后端」行（显示高精度地址 / 本地快速模式）。
