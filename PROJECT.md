...
- **降级**：Karplus-Strong 拨弦物理建模（零采样、不依赖网络），**经典结构：周期延迟 + 两点平均滤波 0.5(x+x·z⁻¹) + 阻尼 0.999**（v2.11.5；0.985/0.99 衰减太快→听感只剩起音「滋」，复现实测 800ms 余音比 0.03→0.43；输出包络按音长淡出、音末强制静音所以不会糊），环外 +3dB@7kHz 提亮（不影响稳定性），默认音量 0.65。诊断面板「采样」行可看到当前是真实采样还是降级拨弦、「采样率」行显示播放上下文采样率、「最近回放」行显示触发数。
