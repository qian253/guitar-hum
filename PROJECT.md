...
- **v2.34.0 调调猫 AI Agent(浮标+猫语大脑)**:右下角猫耳浮标(纯内联SVG手绘,呼吸动画2s,悬停耳朵抖,睡觉闭眼暗示)替代旧🐾召唤按钮——浮标是首页插画那只猫的化身,点击收起原位弹出同一只猫(catSvg)+手绘虚线气泡(点击外部收回)。Agent 三层:工具层 agentTools(最近分析/量化统计/徽章/练习/连续天数)+记忆 AgentMem(localStorage 对话史+事实,给 AI 当上下文)+润色器 agentPolish(分析结果→猫语);本地意图回答为离线小脑;**智谱 GLM-4-Flash 直连**(CORS 已放行;key 存 localStorage.hkAgentKey,设置页「AI 助手」输入行,失败自动落回本地)。自动低语:分析后 2.6s 润色成猫语(低置信让位给安慰猫,与结果页猫互斥);autoWhisper 用本地润色器不烧 token,显式提问才走 AI。API:window.__hkAgent.showAgentResult/ask/memory/polish/tools。已验证:headless Edge 真 API 端到端
- **v2.33.2 主音引擎真实人声校准(12条小星星模板集,前后端同步)**:①起始长音主音加成(0.20/0.15/0.05,镜像结束音)②minor阈值1.5→1.25回滚(Trae基于音准漂移错误前提)③P2三和弦长度门控(n≤5满/6-8半/≥9降1/6——旋律音符集≠和弦,IV级三和弦误赞是小星星类旋律最大误判源)④P3时长门1.0→2.0×(主→属下行误判V→I)⑤decide_mode改音分感知三度能量(±75音分软计数——人声mi常偏低40~80音分,四舍五入把小三度虚高)⑥转调显式切换补实现(文档化「整体判后段目标调」此前缺失;后段margin≥0.15且>前段才切,168基准12例nontonic_end不误切)。成果:后端主音11/12(基线2/12)、前端key.js主音11/12、168合成基准主音100%、全套件通过。测试台:basic-pitch-server/eval12.py(12条真实样本,同调取最后一录)+diag12.py+exp.py(单变量运行时补丁实验器)+test/eval12_js.js(前端对照)。未解:3条大调判小调=歌手mi偏低40~80音分(诚实反映音频);G#/F#重录条内容仍偏离标注调
