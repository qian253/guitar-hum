...
- **降级**：Karplus-Strong 拨弦物理建模（零采样、不依赖网络），**经典结构：周期延迟 + 两点平均滤波 0.5(x+x·z⁻¹) + 阻尼 0.985**（v2.11.2，任意频率增益 ≤1 不可能自激），默认音量 0.65。诊断面板「采样」行可看到当前是真实采样还是降级拨弦、「采样率」行显示播放上下文采样率、「最近回放」行显示触发数与采样/K-S 路径计数。
- **v2.11.3 听旋律音色与重音还原**：旋律回放回归**采样器优先**（正常音色，简谱点按同链路），逐音独立决策、失败即落 K-S，路径计数进诊断面板；**重音还原**：basic-pitch 振幅 → 力度，快速模式无振幅时从**录音 PCM 按每音 [start,end] 窗口算 RMS** → 力度（两种模式都有强弱还原）。
